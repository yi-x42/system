...
from fastapi import APIRouter, HTTPException, Depends, BackgroundTasks, File, UploadFile, Query, WebSocket
...
from app.services.live_person_camera_webrtc import LivePersonCameraVideoTrack
...
class LivePersonCameraRequest(BaseModel):
    """Live Person Camera 請求模型"""
    task_name: str = Field(..., description="任務名稱")
    camera_id: str = Field(..., description="選中的攝影機ID")
    model_path: str = Field("yolov8n.pt", description="YOLO模型路徑")
    confidence_threshold: float = Field(0.35, description="信心度閾值", ge=0.0, le=1.0)
    imgsz: int = Field(640, description="推理圖像大小")
    device: str = Field("", description="設備設定")
    trace_length: int = Field(30, description="軌跡長度")
    heatmap_radius: int = Field(40, description="熱力圖半徑")
    heatmap_opacity: float = Field(0.5, description="熱力圖透明度")
    blur_kernel: int = Field(25, description="模糊核大小")
    csv_path: str = Field("", description="CSV日誌路徑")
    csv_append: bool = Field(False, description="是否追加到CSV")
    corner_enabled: bool = Field(True, description="是否啟用邊框註解")
    blur_enabled: bool = Field(False, description="是否啟用模糊註解")
    trace_enabled: bool = Field(False, description="是否啟用軌跡註解")
    heatmap_enabled: bool = Field(False, description="是否啟用熱力圖註解")
    line_enabled: bool = Field(False, description="是否啟用出入線計數")
    zone_enabled: bool = Field(False, description="是否啟用區域停留統計")
    line_start_x: Optional[int] = Field(None, description="線起始X座標")
    line_start_y: Optional[int] = Field(None, description="線起始Y座標")
    line_end_x: Optional[int] = Field(None, description="線結束X座標")
    line_end_y: Optional[int] = Field(None, description="線結束Y座標")
    description: str = Field("", description="任務描述")

class LivePersonCameraResponse(BaseModel):
    """Live Person Camera 回應模型"""
    task_id: str = Field(..., description="任務ID")
    status: str = Field(..., description="任務狀態")
    message: str = Field(..., description="回應訊息")
    camera_info: Dict[str, Any] = Field(..., description="攝影機資訊")
    config: Dict[str, Any] = Field(..., description="服務配置")
    created_at: datetime = Field(..., description="創建時間")
...
@router.post("/analysis/start-live-person-camera", response_model=LivePersonCameraResponse)
async def start_live_person_camera(
    request: LivePersonCameraRequest,
    background_tasks: BackgroundTasks,
    db: AsyncSession = Depends(get_db)
):
    """開始 Live Person Camera 分析任務"""
    try:
        api_logger.info(f"收到 Live Person Camera 請求: {request}")
        ...
        for cam in cameras:
            if str(cam.id) == str(request.camera_id):
                device_index = cam.device_index if cam.device_index is not None else 0
                camera_stream_id = f"camera_{device_index}"
                camera_info = {
                    "id": cam.id,
                    "name": cam.name,
                    "status": cam.status,
        ...
        model_full_path = resolve_model_path(request.model_path)
        if not model_full_path or not Path(model_full_path).exists():
            raise HTTPException(status_code=404, detail=f"模型檔案不存在: {request.model_path}")
        ...
        try:
            source_info = {
                ...
                "camera_type": camera_info.get("camera_type", "USB"),
                "device_index": camera_info.get("device_index", 0),
                "camera_stream_id": camera_info.get("camera_stream_id"),
                "model_path": str(model_full_path),
                "confidence_threshold": request.confidence_threshold,
                "imgsz": request.imgsz,
                "device": request.device,
                "annotators": {
                    "corner": request.corner_enabled,
                    "blur": request.blur_enabled,
                    "trace": request.trace_enabled,
                    "heatmap": request.heatmap_enabled,
                    "line": request.line_enabled,
                    "zone": request.zone_enabled,
                },
                "line_config": {
                    "start_x": request.line_start_x,
                    "start_y": request.line_start_y,
                    "end_x": request.line_end_x,
                    "end_y": request.line_end_y,
                },
                "csv_path": request.csv_path,
                "csv_append": request.csv_append,
                "analysis_mode": "live_person_camera",
            }
            ...
            analysis_task = AnalysisTask(
                ...
                status="pending",
                source_info=source_info,
                source_width=int(camera_info.get("resolution", "640x480").split("x")[0]) if "x" in str(camera_info.get("resolution", "640x480")) else 640,
                source_height=int(camera_info.get("resolution", "640x480").split("x")[1]) if "x" in str(camera_info.get("resolution", "640x480")) else 480,
                source_fps=camera_info.get("fps", 30.0),
                created_at=datetime.utcnow(),
                task_name=request.task_name,
            ...
            api_logger.info(f"創建 Live Person Camera 任務記錄成功: {task_id}")
        ...
        # 4. 啟動背景 Live Person Camera 任務
        background_tasks.add_task(
            run_live_person_camera,
            task_id=str(task_id),
            camera_info=camera_info,
            request=request,
            db_session_factory=AsyncSessionLocal
        ...
        api_logger.info(f"Live Person Camera 任務 {task_id} 已啟動")

        return LivePersonCameraResponse(
            task_id=str(task_id),
            status="started",
            message="Live Person Camera 任務已成功啟動",
            camera_info={
                "id": camera_info["id"],
                "name": camera_info.get("name", ""),
                "resolution": camera_info.get("resolution", "640x480"),
                "fps": camera_info.get("fps", 30.0)
            },
            config={
                "model_path": str(model_full_path),
                "confidence_threshold": request.confidence_threshold,
        ...
        api_logger.error(f"啟動 Live Person Camera 失敗: {e}")
        raise HTTPException(status_code=500, detail=f"啟動 Live Person Camera 失敗: {str(e)}")

@router.delete("/analysis/live-person-camera/{task_id}")
async def stop_live_person_camera(task_id: str, db: AsyncSession = Depends(get_db)):
    """停止 Live Person Camera 分析任務"""
    try:
        api_logger.info(f"收到停止 Live Person Camera 請求: {task_id}")
        ...
        if source_info.get("analysis_mode") != "live_person_camera":
            raise HTTPException(status_code=400, detail=f"任務 {task_id} 不是 Live Person Camera 任務")

        # 2. 停止服務
        from app.services.live_person_camera_service import live_person_camera_manager
        success = await live_person_camera_manager.stop_service(str(task_id))
        ...
        message = "Live Person Camera 任務已成功停止" if success else "停止 Live Person Camera 任務時發生錯誤"
        api_logger.info(f"{message}: {task_id}")
        ...
        api_logger.error(f"停止 Live Person Camera 失敗: {e}")
        raise HTTPException(status_code=500, detail=f"停止 Live Person Camera 失敗: {str(e)}")
...
async def live_person_camera_websocket(websocket: WebSocket, task_id: str):
    """Live Person Camera WebSocket 串流"""
    from app.services.live_person_camera_service import live_person_camera_manager

    service = live_person_camera_manager.get_service(task_id)

    if not service or not service.running:
        await websocket.accept()
        await websocket.send_json({"type": "error", "message": "任務尚未運行或不存在"})
...
@router.post("/analysis/live-person-camera/{task_id}/webrtc")
async def start_live_person_camera_webrtc(task_id: str, offer: WebRTCOffer):
    """建立 Live Person Camera WebRTC 連線"""
    from app.services.live_person_camera_service import live_person_camera_manager
    ...
    if not service or not service.running:
        for _ in range(30):
            ...
            raise HTTPException(status_code=404, detail=f"任務 {task_id} 尚未運行或不存在")

    pc = RTCPeerConnection()
    await service.register_peer_connection(pc)

    video_track = LivePersonCameraVideoTrack(service)
    ...
    def on_datachannel(channel):
        ...
        async def on_message(message):
            ...
            if payload.get("type") == "toggle":
                name = payload.get("name")
                value = payload.get("value")
                if isinstance(value, bool) and isinstance(name, str):
                    await service.update_annotator(name, value)

    @pc.on("connectionstatechange")
    async def on_connection_state_change():
        if pc.connectionState in {"failed", "closed", "disconnected"}:
            await service.unregister_peer_connection(pc)
    ...
    try:
        ...
        await service.unregister_peer_connection(pc)
        raise HTTPException(status_code=500, detail=f"建立 WebRTC 連線失敗: {e}")

    local_description = pc.localDescription
    return {"sdp": local_description.sdp, "type": local_description.type}

@router.get("/analysis/live-person-camera/{task_id}/stream")
async def stream_live_person_camera(task_id: str):
    """Live Person Camera 任務的即時預覽 (MJPEG)"""
    ...
    async def frame_generator():
        ...
        try:
            while True:
                current_service = live_person_camera_manager.get_service(task_id)
                if not current_service:
                    break

                frame_info = current_service.get_latest_frame()
                if frame_info:
                    ...
                    idle_accumulator += poll_interval

                if (not current_service.running and idle_accumulator > 1.0):
                    break

                await asyncio.sleep(poll_interval)
        finally:
            api_logger.info(f"Live Person Camera 任務 {task_id} 預覽串流結束")
...
camera_webrtc_sessions: Dict[str, Set[RTCPeerConnection]] = {}
...
async def scan_cameras(
    register_new: bool = Query(
        True,
        description="是否將新發現的攝影機自動加入設備列表"
    )
):
    """掃描本機攝影機，並視需求自動註冊為可用設備"""
    try:
        camera_service = CameraService()
        ...
        if register_new:
            ...
            existing_indices = {
                ...
                if cam.camera_type.lower() == "usb" and cam.device_index is not None
            }
            existing_names = {cam.name for cam in existing_cameras}

            for camera_data in discovered:
                device_index = camera_data.get("device_index")
                if device_index is None or device_index in existing_indices:
                    continue

                base_name = camera_data.get("name") or f"USB Camera {device_index}"
                ...
                resolution = camera_data.get("resolution") or "640x480"
                fps_value = camera_data.get("fps")
                try:
                    fps = int(fps_value)
        ...
        if registered_devices:
            message = (
                f"掃描完成，發現 {len(discovered)} 個攝影機，其中新增 {len(registered_devices)} 個"
            )
        else:
...
@router.post("/cameras/{camera_id}/webrtc")
async def start_camera_webrtc(camera_id: str, offer: WebRTCOffer):
    """使用 WebRTC 推送攝影機即時影像"""
    from app.services.camera_stream_manager import camera_stream_manager
    from app.services.camera_webrtc import CameraStreamTrack
    ...
    if device_index is None:
        raise HTTPException(status_code=400, detail="僅支援本機 USB 攝影機的 WebRTC 串流")

    try:
        device_index_int = int(device_index)
    except (TypeError, ValueError):
        raise HTTPException(status_code=400, detail=f"攝影機 {camera_id} 的 device_index 無效: {device_index}")

    camera_stream_id = f"camera_{device_index_int}"
    if not camera_stream_manager.is_stream_running(camera_stream_id):
        started = camera_stream_manager.start_stream(camera_stream_id, device_index_int)
        if not started:
            raise HTTPException(status_code=500, detail=f"攝影機 {device_index_int} 無法啟動串流")

    # 確認攝影機確實有回傳影像
    frame_ready = False
    for _ in range(10):
        latest_frame = camera_stream_manager.get_latest_frame(camera_stream_id)
    ...
    if not frame_ready:
        stats = camera_stream_manager.get_stream_stats(camera_stream_id)
        error_msg = stats.get("last_error") if stats else None
        raise HTTPException(
            status_code=500,
            detail=f"攝影機 {device_index_int} 未回傳影像，錯誤: {error_msg or '未知'}"
        )

    try:
        track = CameraStreamTrack(camera_stream_id)
    except Exception as exc:
        api_logger.error(f"建立攝影機 {camera_id} WebRTC Track 失敗: {exc}")
        raise HTTPException(status_code=500, detail=f"建立 WebRTC 串流失敗: {exc}")

    pc = RTCPeerConnection()
    peers = camera_webrtc_sessions.setdefault(camera_stream_id, set())
    peers.add(pc)
    pc.addTrack(track)
    ...
    async def cleanup() -> None:
        ...
        if cleanup_called:
            return
        cleanup_called = True
        peer_set = camera_webrtc_sessions.get(camera_stream_id)
        if peer_set is not None:
            peer_set.discard(pc)
            if not peer_set:
                camera_webrtc_sessions.pop(camera_stream_id, None)
        try:
            await track.stop()
        except Exception:
            pass
    ...
    try:
        ...
        await cleanup()
        api_logger.error(f"處理攝影機 {camera_id} WebRTC offer 失敗: {exc}")
        raise HTTPException(status_code=500, detail=f"處理 WebRTC Offer 失敗: {exc}")

    local_description = pc.localDescription
    if local_description is None:
        await cleanup()
        raise HTTPException(status_code=500, detail="無法取得本地 SDP 描述")

    api_logger.info(f"攝影機 {camera_id} WebRTC 連線建立完成")
    return {"sdp": local_description.sdp, "type": local_description.type}
...
async def run_live_person_camera(
    task_id: str,
    camera_info: Dict[str, Any],
    request: LivePersonCameraRequest,
    db_session_factory
):
    """執行 Live Person Camera 背景任務"""
    from app.services.live_person_camera_service import (
        LivePersonCameraConfig,
        live_person_camera_manager,
    )

    api_logger.info(f"開始執行 Live Person Camera 任務: {task_id}")
    ...
    try:
        ...
        model_full_path = resolve_model_path(request.model_path)
        if not model_full_path or not Path(model_full_path).exists():
            raise Exception(f"模型檔案不存在: {request.model_path}")

        requested_device = (request.device or "").strip()
        resolved_device = requested_device if requested_device else settings.device

        config = LivePersonCameraConfig(
            source=str(camera_info.get("device_index", 0)),
            model_path=str(model_full_path),
            imgsz=request.imgsz,
            conf_threshold=request.confidence_threshold,
            device=resolved_device,
            trace_length=request.trace_length,
            heatmap_radius=request.heatmap_radius,
            heatmap_opacity=request.heatmap_opacity,
            blur_kernel=request.blur_kernel,
            csv_path=request.csv_path if request.csv_path and request.csv_path.strip() else None,
            csv_append=request.csv_append,
            corner_enabled=request.corner_enabled,
            blur_enabled=request.blur_enabled,
            trace_enabled=request.trace_enabled,
            heatmap_enabled=request.heatmap_enabled,
            line_enabled=request.line_enabled,
            zone_enabled=request.zone_enabled,
            line_start_x=request.line_start_x,
            line_start_y=request.line_start_y,
            line_end_x=request.line_end_x,
            line_end_y=request.line_end_y,
        )

        # 3. 啟動服務
        device_index = camera_info.get("device_index", 0)
        camera_stream_id = camera_info.get("camera_stream_id") or f"camera_{device_index}"

        success = await live_person_camera_manager.start_service(
            task_id=task_id,
        ...
        if success:
            api_logger.info(f"Live Person Camera 任務 {task_id} 啟動成功")
            # 這裡可以添加監控邏輯，等待任務完成或被停止
            # 目前先讓它持續運行，直到被外部停止
        else:
            raise Exception("Live Person Camera 服務啟動失敗")

    except Exception as e:
        api_logger.error(f"Live Person Camera 任務失敗: {e}")
        await update_task_status("failed", str(e))
